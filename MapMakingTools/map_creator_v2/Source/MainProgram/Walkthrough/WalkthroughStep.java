...
package MainProgram.Walkthrough;
 ...
 * @author Stephen
 */
public class WalkthroughStep implements java.io.Serializable
{
    //We use this to tell the Serializer when the class is incompatible with the instances found in tmap save files (when the class is changed and becomes incompatible, change this number)
    private static final long serialVersionUID = -2650950480273462277L;
    
    public static InstructionSet s_instructionsFailedToLoadMessage;
    static
    {
        TreeMap<String, BaseInstructionAction> map = new TreeMap<String, BaseInstructionAction>();
        map.put("Instructions for this step failed to load...", null);
        s_instructionsFailedToLoadMessage = new InstructionSet(map);
    }
    
    private int m_segmentIndex = 0;
    private int m_stepIndex = 0;
    public WalkthroughStep(int segment, int step)
    {
        m_segmentIndex = segment;
        m_stepIndex = step;
    }
    ...
    {
        if(!(obj instanceof WalkthroughStep))
            return false;
        return (((WalkthroughStep)obj).GetSegment() == this.GetSegment() && ((WalkthroughStep)obj).GetStep() ==  this.GetStep());
    }
    ...
    {
        return new StringBuilder().append(m_segmentIndex).append(", ").append(m_stepIndex).toString();
    }
    ...
    public InstructionSet GetInstructionSetForStep(WalkthroughOptions options)
    {
        ...
        return result; //map.put("[not set]", null);
    }
}